...
export const Route = createFileRoute("/auth")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Sign in to save trips." },
    ],
  }),
  component: AuthPage,
});
...
function AuthPage() {
  ...
  const onGoogle = async () => {
    // Lovable's OAuth broker lives at /~oauth/initiate, which only exists on
    // Lovable hosting. Everywhere else (Vercel, local dev) go through Supabase.
    const onLovableHost =
    ...
    const returnTo = `${window.location.origin}/auth${redirect ? `?redirect=${encodeURIComponent(redirect)}` : ""}`;
      ...
      <div className="flex flex-1 items-center justify-center px-6">
        <div className="w-full max-w-md rounded-2xl border border-border bg-card p-8 shadow-glow">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {mode === "signin"
              ? "Sign in to your saved trips."
              : "Save trips and pick up planning anywhere."}
          </p>

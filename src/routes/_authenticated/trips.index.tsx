...
export const Route = createFileRoute("/_authenticated/trips/")({
  head: () => ({ meta: [{ title: "My trips — Wayfinder" }, { name: "robots", content: "noindex" }] }),
  component: TripsPage,
});
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-6xl px-6 py-10">
        <div className="mb-8 flex items-center justify-between">
          ...
          <Link to="/"><Button><Plus className="mr-1 h-4 w-4" /> New trip</Button></Link>
        </div>
        ...
        {data && data.trips.length === 0 && (
          <div className="rounded-2xl border border-dashed border-border p-12 text-center">
            ...
            <Link to="/"><Button className="mt-4">Plan your first trip</Button></Link>
          </div>
        )}

        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {data?.trips.map((t) => (
            <div key={t.id} className="group relative rounded-2xl border border-border bg-card p-5 shadow-soft transition-shadow hover:shadow-glow">
              <Link to="/trips/$tripId" params={{ tripId: t.id }} className="block">
                ...
                {t.destination && <p className="mt-1 flex items-center gap-1 text-sm text-muted-foreground"><MapPin className="h-3 w-3" /> {t.destination}</p>}
                {t.start_date && <p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground"><Calendar className="h-3 w-3" /> {t.start_date} → {t.end_date}</p>}
                {t.budget_cents && <p className="mt-2 text-sm font-medium">Budget {formatMoney(t.budget_cents, t.currency)}</p>}
              </Link>
              <button
                onClick={() => { if (confirm("Delete this trip?")) del.mutate(t.id); }}

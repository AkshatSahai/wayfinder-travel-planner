...
/**
 * Free-text Places search near a point, e.g. "best restaurants" near a day's
 * stops. Generalizes `searchTextCategory`'s request shape — that one is
 * locked to the 7 canned activity-category strings, this takes an arbitrary
 * query — for the itinerary chat's research mode, which needs to ground
 * answers in real places rather than the model's own general knowledge.
 */
export async function searchNearby(
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`places nearby search failed (${res.status}): ${text.slice(0, 200)}`);
  }
  const json = (await res.json()) as { places?: Place[] };
  return (json.places ?? []).map((p) => ({
    name: p.displayName?.text ?? "Unknown",
    description: p.editorialSummary?.text ?? p.primaryTypeDisplayName?.text ?? "",
    rating: p.rating ?? null,
    review_count: p.userRatingCount ?? null,
